import { print, createPrintButton } from "./print.js";

let Config = {
  pixelsPerInch: 96,
  pageHeightInCentimeter: 29.7, // must match 'min-height' from 'css/sheets-of-paper-*.css' being used
  pageMarginBottomInCentimeter: 1 // must match 'padding-bottom' and 'margin-bottom' from 'css/sheets-of-paper-*.css' being used
};
...
function interceptPrint(){
  document.addEventListener('keydown', function(event) {
    ...
    if (event.ctrlKey && event.key === 'p') {
      // Prevent the default action (opening print dialog)
      event.preventDefault();
      // Call custom print function
      window.print();
    }
  });
}
